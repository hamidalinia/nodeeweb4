...
let routeHandle = (app, props = {}) => {
  console.log("routeHandle==>",props.handle)
  if (props && props.front && props.front.routes) {
    let PR = createPublicRoute("", props.front.routes,props.handle);
    app.use("/", PR);
  }
  ...
  props.entity.forEach((en) => {
    if (temp.indexOf(en.name) == -1) {
      ...
      if (en && en.routes) {
        let R = createRoute(en.modelName, en.routes, "/customer/",props.handle);

        app.use("/customer/" + en.name, R);
        if (props.admin) {
          let R2 = createRoute(en.modelName, en.routes, "/admin/",props.handle);
