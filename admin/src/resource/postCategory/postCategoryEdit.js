...
const Form = ({ children, ...rest }) => {
  ...
  function saveForm(values) {
    ...
    API.put("/postCategory/"+values._id, JSON.stringify({ ...values }))
      .then(({ data = {} }) => {
        ...
        if (data._id) {
          ...
          // window.location.href = "/#/postCategory/" + data._id;
...
function save(record) {
  ...
  if (theID)
    API.put("/post/modifyPriceByCat/" + theID, JSON.stringify({ type: type, number: number }))
      .then(({ data = {} }) => {
  ...
  return (
    [<Create {...props}>
      ...
      <ResourceContextProvider value={"post"}>
        <List basePath={"/post/bycat/" + props["id"]} filter={{ id: props["id"], kind: "bycat" }}
