...
export const ChangesForm = (props) => {
    ...
    const updateProduct=async (i,post,prices)=>{
        Block.arrows('.pID-'+post._id);
        
         setTimeout(function() {
            setProgress(i)
            if(post.combinations){
                post.combinations.map((combinition,ic)=>{
                    if(combinition){
            ...
            updateServer(post).then((data)=>{
                newProductPrice.push(data)
                setPro(post.title.fa)
                Block.remove('.pID-'+post._id);
            }).catch(()=>{
                failedProductPrice.push(post)
            })
    ...
    const update = async (prices) =>{
        if(selectedProduct.length !== 0){
            selectedProduct.forEach(function (post, index) {
                updateProduct(index,post,prices)
            });
        }else{
            if(data.length !== 0){
    ...
    const updateServer = async (post) =>{
        API.put("/post/" + post._id, JSON.stringify({ ...post }))
        .then(({ data = {} }) => {
          return data;
        })
        .catch((err) => {
