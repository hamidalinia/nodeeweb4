...
export const courseCategoryShow = (props) => {
    ...
    const getCategoryById = (values) =>{
        ...
        console.log('newCoursePricenewCoursePrice',courseObj);
    }
    const getCourses = async (start,end) =>{
        
        await API.get(`/course/${start}/${end}?_order=DESC`).then(({ data = {} }) => {
            if(data.length !== 0){
                // Block.circle('.courseList');
                
                getCategoryById(data);
            }else{
                // Block.remove('.courseList');
                Loading.remove();
                loop = false;
            }
    ...
    const handlerSelectCourse = (pro) =>{
        setSelectedCourses(selectCourses =>[...selectCourses,pro])
    }
    const onSuccessHandler = async (value) =>{
        if(value){
            ...
            let end = 99;
            while(loop){
                await  getCourses(start,end).then(()=>{
                    start+=100;
                    end+=100;
                }).catch(()=>{
                    loop = false
    ...
    return (
        [<Create {...props}>
            <ChangesForm selectedCourse={selectCourses} data={courses} onSuccess={onSuccessHandler}/>
        </Create>,
    //    <List resource={'course'} >
    <Card className={"width1000"} style={{marginTop:'20px'}}>
    <CardHeader title={'  تعداد محصولات  این دسته بندی  ' +courses.length + 'محصول'}/> <span>{}</span>
    <CardContent>
      <div style={{ height: "auto"}} className={"order-chart"}>
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 650 }} aria-label="simple table">
            ...
            <TableBody className='courseList'>
              {
                courses ? (
                      ...
                      >
                        <TableCell padding="checkbox">
                            <Checkbox
                                ...
                                onChange={(e)=> e.target.checked && handlerSelectCourse(row)}
                            />
                            </TableCell>
                        ...
                        <TableCell align="right">
                            <PriceFiled course={row} key={index}/>
                        </TableCell>
    ...
    const {course} = props;
                                           ...
                                           if (course.type == 'variable') {
                                               ...
                                               if (course.combinations) {
                                                   course.combinations.map((comb, key) => {
                                               ...
                                               if (course.in_stock == true) {
                                                   tt = 'موجود';
                                                   thecl = 'succ';
                                               }
                                               return (<div className={'cobm flex-d cobm'}>
                                                   <div className={'flex-1'}>
                                                       <span>قیمت:</span><span>{course.price && course.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</span>
                                                   </div>
                                                   <div className={'flex-1'}>
                                                       <span>با تخفیف:</span><span>{course.salePrice && course.salePrice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</span>
                                                   </div>
                                                   <div className={'flex-1'}>
                                                   ...
                                                   <div className={'flex-1'}>
                                                       <span>تعداد:</span><span>{course.quantity}</span>
                                                   </div>
                                               </div>)
