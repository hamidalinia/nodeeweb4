...
function GetAddress(props) {
  ...
  const [token, setToken]= useState(store.getState()?.store?.user?.token || '')
  console.log('user token: ', store.getState()?.store?.user)
  let {address: storedAddress} = store.getState().store;
      ...
      buttons: [
        {
          ...
          onClick: async (e) => {
            ...
            if(token && !themeData.guestMode){
              ...
              console.log('address: ', checkOutBillingAddress.add.data)
              storedAddress.push(checkOutBillingAddress.add.data)
              onSetAddress(checkOutBillingAddress.add.data);
              setModals(false);
            }
