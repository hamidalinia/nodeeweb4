  ...
  return (
    <Col tag="aside" className={classes} lg={{ size: 3 }} md={{ size: 4 }}>
            ...
            return (
              <ListGroupItem key={idx} className="d-flex px-3 border-0 wedkuhg">
                <div className={'flex-1 txc'}>
                  {!oneItemPerOrder && <div className={'bge'}>
                    <Button
      ...
      <div className={'fdsdf pl-3 pr-3'} onClick={handleToggleCardbar}>
        {card && card.length > 0 && (() => {
          const hasNotBuyableAlone = card.some(item => item.notBuyableAlone);
          const isSingleNotBuyable = hasNotBuyableAlone && card.length === 1;

          return isSingleNotBuyable ? (
            <div className="alert alert-warning mt-4">
              {t('You must add other items to purchase this product.')}
            </div>
          ) : (
            <Link
              to={'/checkout'}
